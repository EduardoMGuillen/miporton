...
export default async function ResidentialAdminQrPage() {
  ...
  const expiredQrCodes = qrCodes.filter(
    (qr) => qr.isRevoked || qr.validUntil < now || qr.usedCount >= qr.maxUses,
  );
  ...
  return (
    <>
      ...
      <Card>
        <h2 className="mb-4 text-lg font-semibold text-slate-900">QR activos ({activeQrCodes.length})</h2>
        <div className="grid gap-3 md:grid-cols-2">
          {activeQrCodes.map((qr) => (
            <article key={qr.id} className="rounded-xl border border-slate-200 bg-slate-50/70 p-4">
              ...
              <p className="text-xs text-slate-500">Residente: {qr.resident.fullName}</p>
          ...
          {activeQrCodes.length === 0 ? (
            <p className="text-sm text-slate-600">No hay QRs activos en este momento.</p>
          ) : null}
        </div>
      </Card>
      ...
      <Card>
        <details>
          <summary className="cursor-pointer list-none text-lg font-semibold text-slate-900">
            QRs expirados ({expiredQrCodes.length})
          </summary>
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            ...
            {expiredQrCodes.length === 0 ? (
              <p className="text-sm text-slate-600">No hay QRs expirados.</p>
            ) : null}
          </div>
        </details>
      </Card>
